...
RSpec.describe MovieService, :vcr do
  ...
  describe 'instance methods' do
    ...
    describe '#movies_by_ids' do
      it 'returns the full data for the given list of movie ids' do
        user_movies = movie_service.movies_by_ids([11, 13, 278])
        expect(user_movies).to be_an(Array)

        user_movies.each do |movie_data|
          expect(movie_data).to be_a(Hash)
