...
RSpec.describe 'users' do
  before do
    ...
    @viewing_party = @user_1.viewing_parties.create!(
      host: @user_1.name, 
      movie: @movie[:title], 
      duration: @movie[:runtime], 
      viewing_time: @movie[:viewing_time], 
      viewing_date: @movie[:viewing_data], 
  ...
  describe 'dashboard' do
    ...
    it 'lists viewing parties' do
      ...
      expect(page).to have_content(@viewing_party[:viewing_date])
      expect(page).to have_content(@viewing_party[:viewing_time])
    end
  end
end
