...
RSpec.describe "Viewing Party New Page" do
  describe "Sandlot viewing party" do
    ...
    xit "lists movie information" do
      allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@user1)
      @movie = MovieSearch.new.one_movie(11528)
    ...
    xit "has form to create viewing party" do
      allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@user1)
      @movie = MovieSearch.new.one_movie(11528)
      visit new_user_movie_viewing_party_path(@user1.id, @movie.id)
    ...
    xit "can create party" do
      allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@user1)
      @movie = MovieSearch.new.one_movie("sandlot").first
      visit new_user_movie_viewing_party_path(@user1.id, @movie.id)
      date = Date.today
