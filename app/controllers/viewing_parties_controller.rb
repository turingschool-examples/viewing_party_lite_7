...
class ViewingPartiesController < ApplicationController
  ...
  def create
    ...
    @movie = MovieFacade.movie_details(params[:movie_id])
    @users = User.all
    @party = ViewingParty.new(movie_id: params[:movie_id], user_id: params[:user_id],
                              date_time: params[:date_time], duration: @movie.runtime, start_time: params[:date_time])
    ...
    params[:invitees].each do |key, value|
      UserViewingParty.new(user_id: key, viewing_party_id: @party.id).save if value == '1'
    end
