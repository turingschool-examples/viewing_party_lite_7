class MoviesService
  ...
  def top_rated_movies #tested
    get_url("movie/top_rated")
  end

  def search_movies(search_query) #tested
    get_url("search/movie?query=#{search_query}")
  end

  def individual_movie(movie_id) #tested
    get_url("movie/#{movie_id}")
  end

  def get_cast_members(movie_id) #tested
    get_url("movie/#{movie_id}/credits")
  end

  def get_review_info(movie_id) #tested
